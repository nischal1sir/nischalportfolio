...
export default function AdminExperience() {
  ...
  const handleReorder = async (newItems: Experience[]) => {
    setExperiencesData(newItems);
    try {
      await experiencesApi.reorder(newItems.map((item, idx) => ({ id: item.id, order_index: idx })));
      setMessage({ type: 'success', text: 'Experience order updated successfully!' });
    } catch (err) {
    ...
    return (
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-gray-900">Experience Management</h1>
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={addExperience} className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm">
